...
{
      if(mysqli_query($link,"update insurance set status='$status', opTxid='$txid', errorCode='$error_code' , resText='$resText' where id='$id'"))
      {
...
//echo "Total Amount of Distributor: $b<br>";
	...
	{
	   $statuse="REFUND";
	   mysqli_query($link,"update insurance set status='$statuse' where username='$username'"); 
	}
	else
	{
	   mysqli_error($link);
